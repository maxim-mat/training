...
# first plot
titanic["familyGroup"] = titanic["pclass"].astype(str) + " - " + titanic["ticket"].str[:-1] + " - " + titanic[
    "embarked"] + " - " + titanic["fare"].astype(str)
family_losses = titanic.groupby(['familyGroup', 'lastName'])['survived'].apply(lambda x: (x == 0).sum()).reset_index(
    name='count')
